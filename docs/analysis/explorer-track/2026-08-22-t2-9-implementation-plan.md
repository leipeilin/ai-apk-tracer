...
> **任务编号**：T2.9
> **日期**：2026-08-22
> **依据大纲**：`docs/analysis/project-optimization/2026-08-18-project-optimization-plan.md` §2.2（未命中现有 taxonomy 的 sink 标记 custom_sink_proposal，不直接否决，进入 partially_validated 或人工队列）、§2.5（custom sink 升级闭环：人工确认 → 版本化扩展 sink taxonomy → 候选重校验 → validated → 进 golden 集）；实施计划 T2.9 行
