...
> **任务编号**：T2.12
> **日期**：2026-08-22
> **依据大纲**：`docs/analysis/project-optimization/2026-08-18-project-optimization-plan.md` §2.7（分流 M2 试点：探索 validated 必进 + 规则 L2 以核验替代单轮 review、单轮 L2 保留为 A/B 对照与降级基线；降级回退主链永不阻塞；核验预算独立记账第三本账）；M0 审查 §4.2（适配层：verify 输出补齐 L2 其余字段 + evidence_refs 类型转换 context_id 回填）；2026-08-21 决断 3
