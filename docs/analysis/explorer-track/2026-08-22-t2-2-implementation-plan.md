...
> **日期**：2026-08-22
> **依据大纲**：
> - 方案：`docs/analysis/project-optimization/2026-08-18-project-optimization-plan.md` §2.1（六类入口来源表 + 时序约束）+ §2.0 接入原则（L175：api_surface 读产物、不 import 规则侧代码）
> - 实施计划：`docs/analysis/explorer-track/2026-08-21-explorer-track-implementation-plan.md` T2.2（rule_prescan 之后、guard 块后 funnel 前新增阶段）
> - T0.5 设计：`docs/analysis/explorer-track/2026-08-22-t0-5-implementation-plan.md`（schema + 三个对齐决策点：entry_method 依赖 code-index / Binder reliability / 静动态 receiver 区分）
> **状态**：起草
