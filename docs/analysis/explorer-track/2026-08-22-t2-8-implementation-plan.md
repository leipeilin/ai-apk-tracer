...
> **任务编号**：T2.8
> **日期**：2026-08-22
> **依据大纲**：`docs/analysis/project-optimization/2026-08-18-project-optimization-plan.md` §2.4（deep_dive 协议：输入 partial 候选 + 缺失事实清单，输出可回查证据，禁止改写链；预算归属复核账本）、§2.5/§5.4（partial → 深挖或人工高优，不直接进正式 finding）、§4.4/决断 1、§5.5 配置（`max_requests_per_candidate=4`、`deep_dive_prompt_version`）
