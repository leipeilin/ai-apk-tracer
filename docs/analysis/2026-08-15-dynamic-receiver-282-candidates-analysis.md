...
> **勘误（2026-08-15 核验）**：① 原"应用自身代码 0 条（全部为 SDK/第三方库）"断言**错误**——实测应用自身注册点 ≥190 条（67%）：`com/xiaomi/fitness/` 165 + 混淆包 `com/p038mi/` 25（fitness 是应用自身业务包，v2 实证 SportXmsService 等漏洞组件即在此包），SDK/第三方约 92 条。**据此修订建议 4（见 §4）**。② unknown 141→138。③ 复核确认 funnel/ai_required/去重以 findings 库为准：**coverage_insufficient 274 / ai_required=True 275 / representative 唯一 277**（候选侧 chain_id 唯一 282 是 rule-results 阶段计数，未过 funnel 聚合，不作为 findings 侧口径）。核心机制分析与 flag 分级主修复不受影响。
...
282 条是**规则保守性 + 预算错配 + 去重失效**的组合结果，不是误报缺陷。核心修复是 **flag 分级**（exported 送 AI / 未知降 signal），预计送 AI 量 -85%，且不损失真实暴露面（43 条 exported 仍保留，其中多数属应用自身业务包）。基线 run 同规则 76 条同形态，修复全局受益。
