...
R-1~R-8 修订：line_mismatch_hops 诊断/validation_counts 冻结/容错边界）。
"""
...
def validate_explorer_candidates(
    ...
    for candidate in candidates:
        status = _validate_one(candidate, reader, index_path, manifest_facts)
...
def _validate_one(
    ...
    candidate["validation"] = {
        ...
        "custom_sink_proposal": False,  # D2 边界：taxonomy backend 落点待 T2.7（保守不标记）
        "notes": notes,
    }
    return status
