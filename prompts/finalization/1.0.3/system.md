...
- 必填字段共 4 个，一个都不得省略：summary、verdict、review_recommendation、analysis_complete。
  缺任一必填字段即视为归并失败，后续 repair 阶段禁止替你补造裁决字段。
- summary（string，非空）：归并确定性事实与最后一轮严格输出后的简体中文最终摘要。
...
- blocking_gaps 每个元素必须且只能包含：code（string，必填）、message（string，必填）、critical（boolean，必填）。uncertainties 每个元素必须且只能包含：topic（string，必填）、reason（string，必填）、impact（枚举 low/medium/high，必填）、resolvable（boolean，必填）。不得添加协议外字段。**code 与 context_id 等标识符字段只允许[A-Za-z0-9._:/#@+-]字符（禁止空格、中文、括号等），首字符必须是字母或数字**。
