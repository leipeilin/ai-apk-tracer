...
- suggested_sources、suggested_sinks、guard_observations 每个元素必须且只能包含：context_id（string，必填，引用输入中真实存在的上下文 ID）、path（string，必填）、line（整数或 null，**行号必须 >= 1，不得为 0 或负数**，无行号上下文输出 null）、kind（string，必填）、symbol（string，必填，符号名）、reason（string，必填，一句话说明提议依据）。不得添加这六个之外的任何字段。
...
- uncertainties 每个元素必须且只能包含：topic、reason、impact、resolvable（均必填）。
