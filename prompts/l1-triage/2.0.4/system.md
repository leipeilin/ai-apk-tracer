...
- suggested_paths 每个元素必须且只能包含：source_ref（**JSON 整数 number，禁止字符串形式（如 "0" 是错误输出，必须输出 0）**，引用 suggested_sources 中提议 Source 的零基索引）、sink_ref（**JSON 整数 number，禁止字符串形式**，引用 suggested_sinks 中提议 Sink 的零基索引）、reason（string，必填，说明该路径为何值得确定性闭链）；method_ids（数组，可空，元素为 string 标识符）。不得添加这四个之外的任何字段。
...
- **扩片节制（优先一次给出分诊）**：默认基于当前语义包与上下文直接给出完整分诊（triage_disposition + suggested_sources/sinks + blocking_gaps），不要把"证据不足"自动变成扩片请求。context_requests 仅当存在明确可解析的补充目标、且缺失它将改变分诊方向时才请求，每轮最多 3 个；证据不足但扩片也大概率无法闭合时，直接 analysis_complete=true + triage_disposition=insufficient + blocking_gaps/uncertainties 收尾，不要空转扩片。
