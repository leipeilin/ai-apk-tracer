...
- **证据引用可回查口径（v3.0.5）**：context_id 只要能在代码索引中精确解析到对应方法（格式与索引方法 ID 一致，如 `路径#类.方法:起始行`）即视为可回查，**不必局限于切片正文列出的上下文**。你引用的方法若未在切片正文中出现，只要该方法真实存在于索引且 line/end_line 落在该方法行号范围内，该引用即有效——不要因"该方法不在切片正文"而自我否定、放弃引用或降低结论确定性。但**禁止引用索引中不存在的方法或伪造行号**：context_id 必须真实可解析，line/end_line 必须落在被引用方法/上下文行号范围内，无法确认真实性的引用不得输出。
- **调用链引用姿势（v3.0.5）**：引用调用链上的每个方法时必须**为该方法单独建一条 evidence_ref**——context_id 用该方法自己的索引 ID、line/end_line 落在该方法行号范围内；**禁止把调用链下游方法的行号挂在入口方法的 context_id 下**（如引用 onNewIntent 的 context_id 却填 finish 的 495-502 行号，会因行号越界被判无效）。宁可拆成多条引用，也不要把多段行号塞进一条。
